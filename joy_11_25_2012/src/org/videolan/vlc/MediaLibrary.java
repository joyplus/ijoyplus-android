...
public class MediaLibrary {
	...
	public ArrayList<Media> getAudioItems(String name, String name2, int mode) {
		...
		for (int i = 0; i < mItemList.size(); i++) {
			...
			if (item.getType() == Media.TYPE_AUDIO) {
				...
				boolean valid = false;
				/*
				 * switch (mode) { case AudioBrowserFragment.MODE_ARTIST: valid
				 * = name.equals(item.getArtist()) && (name2 == null ||
				 * name2.equals(item.getAlbum())); break; case
				 * AudioBrowserFragment.MODE_ALBUM: valid =
				 * name.equals(item.getAlbum()); break; case
				 * AudioBrowserFragment.MODE_GENRE: valid =
				 * name.equals(item.getGenre()) && (name2 == null ||
				 * name2.equals(item.getAlbum())); break; default: break; }
				 */
				if (valid)
					audioItems.add(item);
	...
	private class GetMediaItemsRunnable implements Runnable {
		...
		public void run() {
			...
			final DatabaseManager DBManager = DatabaseManager.getInstance(App
					.getAppContext());
			...
			// use the external storage as our default root directory (most
			// often /mnt/sdcard)
			if (root == null) {
			...
			try {
				...
				while (!directories.isEmpty()) {
					...
					try {
						...
						// listFiles can fail in OutOfMemoryError, go to the
						// next folder
						continue;
					}
				...
				for (File file : mediaToScan) {
					...
					// MainActivity.sendTextInfo(mContext, file.getName(),
					// count,
					// mediaToScan.size());
					count++;
					if (existingMedias.containsKey(fileURI)) {
						/**
						 * only add file if it is not already in the list. eg.
						 * if user select an subfolder as well
						 */
				...
				// remove old files & folders from database if storage is
				// mounted
