...
public class AudioService extends Service {
	...
	private void setUpRemoteControlClient() {
		...
		// ComponentName eventReceiverComponent = new
		...
		// mediaButtonIntent, 0);
		/*
		 * // create and register the remote control client mRemoteControlClient
		...
		audioFocusListener = new OnAudioFocusChangeListener() {
			...
			public void onAudioFocusChange(int focusChange) {
				...
				if (focusChange == AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK
					...
					/*
					 * Lower the volume to 19% to "duck" when an alert or
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equalsIgnoreCase(ACTION_WIDGET_PLAY)) {
				if (mLibVLC.isPlaying() && mCurrentMedia != null) {
					...
					Intent iVlc = new Intent(context, DetailActivity.class);
			 ...
			 */
				...
				case KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE:
					...
					break;
				/*
				 * case KeyEvent.KEYCODE_MEDIA_PLAY: play(); break; case
				 * KeyEvent.KEYCODE_MEDIA_PAUSE: pause(); break;
				 */
			...
			if (mDetectHeadset) {
				if (action
						.equalsIgnoreCase(AudioManager.ACTION_AUDIO_BECOMING_NOISY)) {
					Log.i(TAG, "Headset Removed.");
					if (mLibVLC.isPlaying() && mCurrentMedia != null)
						pause();
				} else if (action.equalsIgnoreCase(Intent.ACTION_HEADSET_PLUG)
						&& state != 0) {
	...
	private static class AudioServiceEventHandler extends
		...
		public void handleMessage(Message msg) {
			...
			/*
			 * case EventManager.MediaPlayerPlaying: Log.i(TAG,
			 * "MediaPlayerPlaying"); service.changeAudioFocus(true);
			 * service.setRemoteControlClientPlaybackState
			 * (RemoteControlClient.PLAYSTATE_PLAYING); break; case
			 * EventManager.MediaPlayerPaused: Log.i(TAG, "MediaPlayerPaused");
			 * service.executeUpdate(); // also hide notification if phone
			 * ringing service.hideNotification();
			 * service.setRemoteControlClientPlaybackState
			 * (RemoteControlClient.PLAYSTATE_PAUSED); break; case
			 * EventManager.MediaPlayerStopped: Log.i(TAG,
			 * "MediaPlayerStopped"); service.executeUpdate();
			 * service.setRemoteControlClientPlaybackState
			 * (RemoteControlClient.PLAYSTATE_STOPPED); break;
			 */
	...
	private void handleVout() {
		...
		// VideoPlayerActivity.start(VLCApplication.getAppContext(),
	...
	private void showNotification() {
		/*
		 * try { // add notification to status bar NotificationCompat.Builder
		 * builder = new NotificationCompat.Builder(this)
		 * .setSmallIcon(R.drawable.icon) .setLargeIcon(AudioUtil.getCover(this,
		 * mCurrentMedia, 64)) .setContentTitle(mCurrentMedia.getTitle())
		 * .setContentText((Util.isJellyBeanOrLater() ?
		 * mCurrentMedia.getArtist() : mCurrentMedia.getArtist() + " - " +
		 * mCurrentMedia.getAlbum())) .setContentInfo(mCurrentMedia.getAlbum())
		 ...
		 * 
		 * builder.setContentIntent(pendingIntent); //startForeground(3,
		 * builder.build()); } catch (NoSuchMethodError e){ // Compat library is
		 * wrong on 3.2 //
	...
	private final IAudioService.Stub mInterface = new IAudioService.Stub() {
		...
		public void load(List<String> mediaPathList, int position,
				boolean libvlcBacked) throws RemoteException {
		...
		public void showWithoutParse(String URI) throws RemoteException {
			...
			// connection
			mCurrentMedia = new Media(getApplicationContext(), URI, 0, 0,
					Media.TYPE_AUDIO, null, URI, App.getAppContext().getString(
							R.string.unknown_artist), App.getAppContext()
							.getString(R.string.unknown_genre), App
							.getAppContext().getString(R.string.unknown_album),
					0, 0, "");
	...
	private void updateWidget(Context context) {
		...
		Bitmap cover = mCurrentMedia != null ? AudioUtil.getCover(this,
				mCurrentMedia, 64) : null;
