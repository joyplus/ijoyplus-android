...
public class LibVLC {
	...
	public native void attachSurface(Surface surface,
			VideoPlayerActivity player, int width, int height);
	...
	public static synchronized void useIOMX(Context context) {
		...
		try {
			sAout = Integer.parseInt(pref.getString("aout",
					String.valueOf(AOUT_AUDIOTRACK_JAVA)));
	...
	public boolean timeStretchingEnabled() {
		SharedPreferences p = PreferenceManager.getDefaultSharedPreferences(App
				.getAppContext());
	 ...
	 */
	private native byte[] getThumbnail(long instance, String mrl, int i_width,
			int i_height);
	...
	public static native void nativeReadDirectory(String path,
