...
public class Dao_Cache {
	 ...
	 * 查看数据库中是否有数据
	 */
	public synchronized boolean isHasInfors(String prod_id, String prod_subname) {
		SQLiteDatabase database = getConnection();
		int count = -1;
		Cursor cursor = null;
		try {
			String sql = "select count(*)  from video_cache where prod_id=? and prod_subname=?";
	...
	public synchronized void InsertOneInfo(videoCacheInfo info) {
		...
		try {
			...
			Object[] bindArgs = {info.getProd_id(),info.getProd_value(),info.getProd_type(),info.getClass(),info.getProd_subname(),info.getLast_playtime()};
	...
	public synchronized void updateOneInfo(videoCacheInfo info){
		SQLiteDatabase database = getConnection();
		try{
			String sql = "update video_cache set prod_value=? and create_date=? and last_playtime=? where prod_id=? and prod_subname=?";
	 ...
	 * 获取某一条缓存记录
	 */
	public synchronized videoCacheInfo getOneInfo(String prod_id, String prod_subname) {
		videoCacheInfo info = null;
		SQLiteDatabase database = getConnection();
		Cursor cursor = null;
		try {
			String sql = "select prod_id,prod_value, prod_type,create_date,prod_subname,last_playtime from video_cache where prod_id=? and prod_subname=?";
			cursor = database.rawQuery(sql, new String[] { prod_id, prod_subname });
			while (cursor.moveToNext()) {
				info = new videoCacheInfo(cursor.getString(0), cursor.getString(1),
	 ...
	 * 删除某一个记录
	 */
	public synchronized void delete(String prod_id, String prod_subname) {
		SQLiteDatabase database = getConnection();
		try {
			database.delete("video_cache", "prod_id=? and prod_subname=?",
					new String[] { prod_id, prod_subname });
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			if (null != database) {
