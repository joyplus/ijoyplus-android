...
public class BitmapZoom {
	...
	public static Bitmap bitmapZoomByWidth(Bitmap srcBitmap, int newWidth) {
		...
		int srcWidth = srcBitmap.getWidth(); // int srcHeight =
		srcBitmap.getHeight();
	 ...
	 * @return
	 */
	public static Bitmap bitmapZoomByWidthA(Bitmap srcBitmap, int newWidth) {
		...
		if (width > height) {
			...
			scale = (float) Constant.newHeight / height;
		}

		Matrix matrix = new Matrix();
		matrix.postScale(scale, scale);
		// 产生缩放后的Bitmap对象
		Bitmap resizeBitmap = Bitmap.createBitmap(srcBitmap, 0, 0, width,
				height, matrix, true);
