...
public class Util {
	...
	public static String readAsset(String assetName, String defaultS) {
		try {
			InputStream is = App.getAppResources().getAssets().open(assetName);
	...
	public static String getValue(String string, int defaultId) {
		return (string != null && string.length() > 0) ? string : App
				.getAppContext().getString(defaultId);
	}

	public static void setItemBackground(View v, int position) {
		v.setBackgroundResource(position % 2 == 0 ? R.drawable.background_item1
	...
	public static int convertDpToPx(int dp) {
		return Math.round(TypedValue.applyDimension(
				TypedValue.COMPLEX_UNIT_DIP, dp, App.getAppResources()
						.getDisplayMetrics()));
	}
	...
	public static boolean isJellyBeanOrLater() {
		return false;// android.os.Build.VERSION.SDK_INT >=
	...
	public static boolean hasCompatibleCPU() {
		...
		try {
			properties.load(new ByteArrayInputStream(Util.readAsset("env.txt",
					"").getBytes("UTF-8")));
		} catch (IOException e) {
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (line.contains("clflush size"))
					hasX86 = true;
				// TODO: MIPS - "microsecond timers"; see
